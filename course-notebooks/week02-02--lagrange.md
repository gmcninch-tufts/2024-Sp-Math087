::: {#7b6de82e-6b77-4ff6-b685-105ef2f84445 .cell .markdown}
:::

::: {#63b091ac-a60c-4859-ac65-231ef7cf266c .cell .markdown}
...
::: {#29f73841-bf7f-4b7b-8355-62adee4aeb0b .cell .markdown}
# Some motivation
...
must also check boundary points (endpoints).
...
::: {#5821332f-965b-4cec-a7ec-ca8a1f1a23db .cell .markdown}
# Lagrange multipliers {#lagrange-multipliers}
...
We introduce a "new" function -- now of *three* variables - known as the
**Lagrangian**. It is given by the formula $$F(x,y,\lambda) =
...
(Recall that
$\dfrac{\partial f}{\partial x} \mathbf{i} + \dfrac{\partial f}{\partial y} \mathbf{j}$
is the *gradient* $\nabla f$ of $f$).
...
(and for some scalar $\mu$). And we must argue that $\mathbf{v}$ is
zero.
...
::: {#7ef8057c-8c6f-4aab-853f-92a1ef3d470a .cell .markdown}
# Televisions, again
...
$F(s,t,\lambda) = P(s,t) - \lambda(s+t-10,000).$
...
::: {#ec4cd95c-d167-4aa3-b7e5-39fa8f269a3b .cell .code}
``` python
import numpy as np
...
::: {#887f3020-1856-4ddc-b073-4d8a2aa5da30 .cell .markdown}
We find that $s \approx 3,846$, $t \approx 6,154$ and $λ = 24$.
:::

::: {#33062a77-b5c2-4d48-bae0-5f0c04b5bf0d .cell .markdown}
Now, effectively we consider the profit function $P$ subject to the
constraint $s+t=10,000$ where $s \ge 0$ and $t \ge 0$. On this "closed
interval", the function $P$ will assume a maximum and a minimum value.
We've found the "critical point" of $P$ on this "interval" -- namley
$(3846, 6154)$. The *endpoints* of the interval are $(0,10000)$ and
$(10000,0)$.

Let's compare the values of $P$ at these points of interest:
:::

::: {#2f910652-1132-471a-a675-7ab9861978d3 .cell .code}
``` python
def p(s,t):
...
::: {#ba3c911f-7d07-47cf-9f87-f95114a468e7 .cell .markdown}
...
::: {#30a56a66-86ad-4739-b283-32d99d95463f .cell .markdown}
...
coefficient matrix. We must also introduce a `symbol` for the unknown
`c`.
:::

::: {#af1a1330-cc5d-4d08-9fd4-9f3fa5f9e1dc .cell .code}
``` python
import numpy as np
import sympy as sp
...
::: {#c2d5b634-d990-4b6e-9022-5e0c00c79316 .cell .markdown}
...
$$S(s,c=10000) \approx 1.3 \quad S(t,c=10000) \approx 0.8$$ which shows
...
or just rewrite $P$ as a function of $c$. After some calculation, we
find that $$S(P,c=10000) \approx 24 \cdot
\dfrac{10000}{532308} \approx 0.45.$$ So a 1% increase in $c$ yields a
$0.45%$ increase in $P$.

Interestingly, observe that $$\dfrac{dP}{dc} = \lambda.$$

Let's pause to recall that with the method of Lagrange multipliers, the
value of gradient of the function we are optimizing -- $P$ in this case
...
Thus, the Lagrange multiplier, λ actually has a "physical" meaning here.
...
is less than the additional profit, you probably should go for it!
:::
